import { useToggle } from "../context/toggleContext";
import Button from "./Button";

const Cart = ({ setShowCart }: { setShowCart: (show: boolean) => void }) => {
  const { cart, removeFromCart } = useToggle();

  const handleCheckout = () => {
    if (cart.length === 0) {
      alert("no item added");
    } else {
      confirm("Do you want to make payment?");
      alert("Item purchased");
      cart.splice(0);
    }

    setShowCart(false);
  };

  return (
    <article className="">
      <div className="p-6 bg-[var(--color-white)] rounded-lg shadow-md max-w-[30rem] grid gap-4">
        <h3 className="font-bold text-2xl pb-6 border-b-4 border-[var(--color-dark-grayish-blue)]">
          Cart
        </h3>

        {!cart.length ? (
          <p className="text-center">The cart is empty</p>
        ) : (
          cart.map((item) => {
            return (
              <div className="flex gap-4 items-center">
                <img
                  src={item.image}
                  alt={item.image}
                  className="w-[5rem] h-[5rem] rounded-lg"
                />
                <div className="flex flex-col gap-2 flex-grow">
                  <p>{item.title}</p>
                  <p>
                    ${item.price} X {item.quantity}{" "}
                    <span className="font-bold text-2xl space-x-8">
                      ${item.price * item.quantity}
                    </span>
                  </p>
                </div>
                <span onClick={() => removeFromCart(item)}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
                    viewBox="0 0 24 24"
                    strokeWidth="1.5"
                    stroke="currentColor"
                    className="size-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
                    />
                  </svg>
                </span>
              </div>
            );
          })
        )}

        <Button
          variant="primary"
          text="Checkout"
          onClick={() => handleCheckout()}
        />
      </div>
    </article>
  );
};

export default Cart;
